...
class DatasetClient(Hydrator):
    ...
    def links(self, model: type[BaseModel]) -> pd.DataFrame:
        ...
        rows = []
        names = {
            getattr(cls, "rdf_class_iri", ""): name for name, cls in self.models.items()
        }
        fields = {}
    ...
    def search(
        self, model: type[Model], text: str, *,
        ...
        rows = self._select(
            f"SELECT DISTINCT ?s WHERE {{ {body} }} ORDER BY ?s LIMIT {limit + 1}"
        )
        if len(rows) > limit:
    ...
    def follow(
        self, records: list[BaseModel], field: str, target: type[Model], *,
        fields: list[str] | None = None, inverse: bool = False,
    ) -> list[Model]:
        ...
        for start in range(0, len(subjects), self.batch_size):
            ...
            for row in rows:
                source_term, target_term = _term(row.get("source", {})), _term(row.get("target", {}))
                if source_term.kind != "uri" or source_term.value not in batch or target_term.kind != "uri":
                    raise EndpointError("Unexpected source or target in link results")
                targets.add(target_term.value)
                self._matches.append({
                    "source": source_term.value, "target": target_term.value,
                    "field": field, "path": path.model_dump(mode="json"),
