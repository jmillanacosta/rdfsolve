...
def search_records(
    ...
    if fields and any(
        (model.model_fields[client.field_name(model, name)].json_schema_extra or {})
        .get("rdf_path", {})
        .get("operator")
...
def _search_field_paths(client, terms, models, fields):
    ...
    for model in models:
        ...
        for field in fields:
            ...
            branches.append(
                "{ " + f"?s a {_iri(cls)} ; {path_to_sparql(path)} ?text . "
    ...
    with client.step("Search selected generated field paths"):
        rows = client._select(
            "SELECT DISTINCT ?s ?type ?field ?text ?_graph WHERE { "
    ...
    return Results(
        ...
        coverage={
            ...
            "basis": "Text candidates through explicitly selected generated field paths",
