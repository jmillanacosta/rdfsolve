...
class VoidParser:
    ...
    def _extract_about_metadata(
        ...
        # Use explicit values, use graph values
        if endpoint_url:
    ...
    def to_mined_schema(self) -> Any:
        ...
        Reconstructs the internal MinedSchema object from the VoID RDF graph.
        This enables: MinedSchema -> VoID RDF -> JSON-LD -> MinedSchema
        ...
        # Use configured base URI for rdfsolve namespace
        base_uri = get_base_uri()
        rdfsolve_ns = URIRef(f"{base_uri}/vocab#")

        # Find the void:Dataset node
        dataset_uri = None
        for s, _p, _o in self.graph.triples((None, RDF.type, URIRef(f"{void_ns}Dataset"))):
            dataset_uri = s
            break

        if not dataset_uri:
            raise ValueError("No void:Dataset found in VoID graph")

        # Helper to get literal value
        def get_literal(pred: URIRef) -> str | None:
            """Extract literal string value for predicate."""
            for _, _, o in self.graph.triples((dataset_uri, pred, None)):
                return str(o)
            return None

        # Helper to get integer value
        def get_int(pred: URIRef) -> int | None:
            """Extract integer value for predicate."""
            val = get_literal(pred)
            return int(val) if val else None

        # Helper to get float value
        def get_float(pred: URIRef) -> float | None:
            """Extract float value for predicate."""
            val = get_literal(pred)
        ...
        about_data["endpoint"] = get_literal(URIRef(f"{void_ns}sparqlEndpoint"))
        about_data["title"] = get_literal(DCTERMS.title)
        about_data["description"] = get_literal(DCTERMS.description)

        # Source metadata
        about_data["source_license"] = get_literal(DCTERMS.license)
        about_data["source_publisher"] = get_literal(DCTERMS.publisher)
        about_data["homepage"] = get_literal(FOAF.homepage)
        about_data["source_version"] = get_literal(URIRef(f"{void_ns}vocabulary"))
        about_data["source_version_iri"] = get_literal(OWL.versionIRI)
        about_data["source_issued"] = get_literal(DCTERMS.issued)
        about_data["source_modified"] = get_literal(DCTERMS.modified)

        # Creators (multiple allowed)
        creators = []
        for _, _, o in self.graph.triples((dataset_uri, DCTERMS.creator, None)):
            creators.append(str(o))
        if creators:
            about_data["source_creator"] = creators

        # Tool provenance
        about_data["generated_by"] = get_literal(URIRef(f"{rdfsolve_ns}generatedBy"))
        about_data["generated_at"] = get_literal(DCTERMS.created)
        about_data["strategy"] = get_literal(URIRef(f"{rdfsolve_ns}miningStrategy"))
        about_data["rdfsolve_version"] = get_literal(URIRef(f"{rdfsolve_ns}toolVersion"))

        # Schema identity
        about_data["schema_id"] = get_literal(URIRef(f"{rdfsolve_ns}schemaId"))
        about_data["schema_version"] = get_literal(URIRef(f"{rdfsolve_ns}schemaVersion"))

        # Timing
        about_data["started_at"] = get_literal(URIRef(f"{rdfsolve_ns}miningStartedAt"))
        about_data["finished_at"] = get_literal(URIRef(f"{rdfsolve_ns}miningFinishedAt"))
        about_data["total_duration_s"] = get_float(URIRef(f"{rdfsolve_ns}miningDurationSeconds"))

        # Statistics (provide defaults for non-optional int fields)
        about_data["pattern_count"] = get_int(URIRef(f"{rdfsolve_ns}patternCount")) or 0
        about_data["class_count"] = get_int(URIRef(f"{void_ns}classes")) or 0
        about_data["property_count"] = get_int(URIRef(f"{void_ns}properties")) or 0
        about_data["declared_class_count"] = (
            get_int(URIRef(f"{rdfsolve_ns}declaredClassCount")) or 0
        )
        about_data["used_type_count"] = get_int(URIRef(f"{rdfsolve_ns}usedTypeCount")) or 0

        # Filter None values
        about_data = {k: v for k, v in about_data.items() if v is not None}

        about = AboutMetadata(**about_data)

        # Extract patterns from property partitions
        patterns = []
        for partition, _, prop_uri in self.graph.triples(
            (None, URIRef(f"{void_ns}property"), None)
        ):
            # Get subject class
            subject_class = None
            for _, _, sc in self.graph.triples(
                (partition, URIRef(f"{void_ext_ns}subjectClass"), None)
            ):
                subject_class = str(sc)
                break

            # Get object class
            object_class = None
            for _, _, oc in self.graph.triples(
                (partition, URIRef(f"{void_ext_ns}objectClass"), None)
            ):
                object_class = str(oc)
                break
            ...
            if subject_class:
                for _, _, lbl in self.graph.triples(
                    (
            ...
            if prop_uri:
                for _, _, lbl in self.graph.triples(
                    ...
                    property_label = str(lbl)
                    break

            if object_class and object_class not in ("Literal", "Resource", "BlankNode"):
                for _, _, lbl in self.graph.triples(
                    (
                        URIRef(object_class),
                        URIRef("http://www.w3.org/2000/01/rdf-schema#label"),
