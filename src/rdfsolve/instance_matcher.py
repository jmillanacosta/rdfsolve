...
def _get_uri_formats(prefix: str) -> list[str]:
    ...
    # Resolver/redirect hosts are not real entity namespaces and produce
    # false positives (or wasted queries) when used as STRSTARTS patterns.
    _RESOLVER_HOSTS = ("bioregistry.io/", "n2t.net/", "identifiers.org/")
    ...
    for prefix_str in sorted(raw):  # sorted for deterministic order
        if not prefix_str:
            continue
        if any(h in prefix_str for h in _RESOLVER_HOSTS):
            continue
