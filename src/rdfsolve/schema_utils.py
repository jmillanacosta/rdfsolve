...
class SchemaAnalyzer:
    ...
    def generate_linkml_schema(
        self, 
        schema_suffix: str = "",
        use_jsonld_conversion: bool = True,
        ...
        if save_to_file:
            linkml_file = os.path.join(
                self.exports_path, f"{schema_name}.yaml"
            )
    ...
    def compare_schemas(
        ...
        return {
            ...
            'schema1_only_examples': sorted(list(slots1 - slots2))[:3],
            'schema2_only_examples': sorted(list(slots2 - slots1))[:3]
        }
    
    def display_schema_summary(
        self, 
        sv: SchemaView, 
        title: str = "Schema Summary"
    ) -> Dict[str, int]:
...
class CoverageVisualizer:
    ...
    def _create_figure(self, df: pd.DataFrame) -> go.Figure:
        ...
        text_positions = [
            "outside" if v < 95 else "inside" 
            for v in df["coverage_percent"]
        ]
        ...
        fig = go.Figure(
            go.Bar(
                ...
                hovertemplate=(
                    "<b>%{y}</b><br>Coverage: %{x:.1f}%<extra></extra>"
                ),
            )
        )
    ...
    def _configure_layout(self, fig: go.Figure, fig_height: int) -> None:
        ...
        fig.update_layout(
            ...
            autosize=True,
            height=fig_height,
            margin=dict(t=80, b=50, l=480, r=150),
            plot_bgcolor="white",
            paper_bgcolor="white",
        )
...
class PydanticModelAnalyzer:
    ...
    def display_all_models(cls, pydantic_models: dict) -> None:
        ...
        markdown_output = (
            f"**All {len(pydantic_models)} generated "
            f"Pydantic classes:**\n\n"
        )
        
        for name in sorted(pydantic_models.keys()):
            markdown_output += f"### {name}\n"
            markdown_output += cls.show_model_fields(
                pydantic_models[name]
            ) + "\n\n"
