...
def test_vocabulary_declarations_become_models_and_rdf():
    schema = MinedSchema.from_vocabulary(VOCABULARY, classes=[S.Person, S.Organization])
    ...
    org = client.create("Organization", uri="urn:org", name="Institute")
    ada = client.create(
        "Person", uri="urn:ada", name="Ada", birthDate="1815", rank=1, affiliation=org,
        sameAs=URIRef("https://www.wikidata.org/wiki/Q7259"),
    )
    graph = ada.to_graph() + org.to_graph()
    ada_iri = URIRef("urn:ada")
    assert graph.value(ada_iri, S.birthDate) == Literal("1815", datatype=XSD.gYear), "Partial ISO dates"
