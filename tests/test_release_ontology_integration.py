...
def test_release_links_global_ontology_registry_and_dataset_usage(tmp_path: Path):
    ...
    (d / "demo_ontology_acquisition.json").write_text(json.dumps({
        "dataset_id":"demo", "infrastructure_namespaces":[], "candidates":[{
            "namespace":"http://purl.obolibrary.org/obo/TEST_", "ontology_id":"test",
            "observed_classes":["http://purl.obolibrary.org/obo/TEST_1"], "observed_properties":[],
            "provider_graphs":[], "reference_sources":[{"source_url":"https://example.org/test.owl"}],
            "identity_basis":"reference_source"
        }]
    }))
    (d / "demo_ontology_usage.json").write_text(json.dumps({"dataset_id":"demo","usages":[{
        "dataset_id":"demo", "ontology_artifact_id":"sha256:abc", "ontology_id":"test",
        "namespace":"http://purl.obolibrary.org/obo/TEST_", "artifact_relation":"reference_release",
        "version_match_status":"unknown", "resolved_classes":["http://purl.obolibrary.org/obo/TEST_1"],
        "unresolved_classes":[], "resolved_properties":[], "unresolved_properties":[], "term_usage":[]
    }]}))
    o=tmp_path / "ontologies"; o.mkdir()
