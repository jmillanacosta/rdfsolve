"""Run graph queries through the pipeline and inspect the release files."""
...
def test_graph_pipeline_release(tmp_path, monkeypatch, mode, state):
    ...
    link = next(p for p in schemas[0].patterns if p.subject_class == str(E.A) and p.property_uri == str(E.link))
    ...
    for path in schema_paths:
        report = json.loads(path.with_name(path.name.replace("_schema.json", "_report.json")).read_text())
    ...
    if mode == "grouped":
        ...
        for index, graph in enumerate([left_graph, right_graph]):
            expected = {(p.subject_class, p.property_uri, p.object_class, p.datatype): p.graphs[graph]
                        for p in group.patterns if p.graphs and graph in p.graphs}
            actual = {(p.subject_class, p.property_uri, p.object_class, p.datatype): p.count
        ...
        text = next(p for p in schemas[0].patterns if p.subject_class == str(E.A) and p.property_uri == str(E.text))
        assert text.count == 2
    ...
    for schema in schemas:
        assert all(not any(str(term).startswith("http://www.w3.org/ns/shacl#") or "sparql-examples" in str(term)
